...
describe("product workbench service", () => {
  ...
  it("generates image metadata with GPT5.5 while preserving manual fields", async () => {
    const root = setupWorkbenchRoot();
    const batchId = writeDesktopBatch(root);
    const fetchMock = vi.fn(async (_url: string, init: RequestInit) => {
      const body = String(init.body);
      expect(body).toContain("imageMetas");
      expect(body).toContain("rabbit-pink");
      ...
      return new Response(JSON.stringify({
        choices: [{ message: { content: JSON.stringify({
          imageMetas: [
            { itemId: "item_1", color: "GPT pink", size: "18 cm", material: "soft plush", note: "floral ears" },
            { itemId: "item_2", color: "gray", size: "20 cm", material: "plush fabric", note: "blue floral accents" },
          ],
        }) } }],
      }), {
        status: 200,
    ...
    vi.stubGlobal("fetch", fetchMock);
    const service = await import("../productWorkbenchService.js");
    service.updateProductWorkbenchImageMetas(batchId, [{
      itemId: "item_1",
      color: "manual pink",
    }]);

    const updated = await service.generateProductWorkbenchImageMetas(batchId);
    ...
    expect(updated.imageMetas[1]).toMatchObject({
      itemId: "item_2",
      color: "gray",
      size: "20 cm",
      material: "plush fabric",
      note: "blue floral accents",
      source: "gpt55",
    });
  });
});
